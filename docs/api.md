...
Match an error by name or code. The `_` key is the fallback. Only fault-created errors (with `isFault === true`) are matched by name/code — plain `Error` objects fall through to `_`.
